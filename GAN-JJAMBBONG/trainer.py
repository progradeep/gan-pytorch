    ...
    def train(self):
        ...
        for epoch in range(self.train_batches):
            ...
            for step in range(len(self.video_loader)):
                ...
                ############################
                # (1) Update G network: minimize Lgan(MSE) + info loss
                ###########################
                ...
                fakeFG, generated_categ = fake[0][0], fake[0][1]
                fakeBG = self.generator.sample_background(image_batch_size)
                fakeMask = self.generator.sample_masks(image_batch_size)

                fakeGif = fakeMask * fakeFG + (1 - fakeMask) * fakeBG

                output, fake_categ = self.video_discriminator(fakeGif)

                loss_G = self.gan_criterion(output, Variable(torch.ones(output.size()).cuda()))

                # L1 loss: first frame - real image
                ...
                #### train with image
                # fakeIm = fake[1][0]
                r = random.randrange(0, image_batch_size)
                fakeIm = fakeGif[:,:,r,:,:]
                ...
                ############################
                # (2) Update D network: minimize LSGAN loss + category loss
                ###########################
                ...
                if step % self.log_interval == 0:
                    ...
                    fakeBG = self.generator.sample_background(self.image_batch_size)
                    fakeMask = self.generator.sample_masks(self.image_batch_size)

                    fakeGif = fakeMask * fakeFG + (1 - fakeMask) * fakeBG

                    fakeFG = fakeFG.permute(0, 2, 1, 3, 4)
                    fakeFG = fakeFG.resize(self.video_batch_size * self.video_length, self.n_channels,
                                             self.image_size, self.image_size)
                    vutils.save_image(denorm(fakeFG.data), '%s/fakeFG_%03d_%d.png' % (self.outf, epoch, step),
                                      nrow=self.video_length)

                    fakeBG = fakeBG.permute(0, 2, 1, 3, 4)
                    fakeBG = fakeBG.resize(self.video_batch_size, self.n_channels,
                                           self.image_size, self.image_size)
                    vutils.save_image(denorm(fakeBG.data), '%s/fakeBG_%03d_%d.png' % (self.outf, epoch, step),
                                      nrow=1)

                    fakeMask = fakeMask.permute(0, 2, 1, 3, 4)
                    fakeMask = fakeMask.resize(self.video_batch_size * self.video_length, 1,
                                           self.image_size, self.image_size)
                    vutils.save_image(denorm(fakeMask.data), '%s/fakeMask_%03d_%d.png' % (self.outf, epoch, step),
                                      nrow=self.video_length)
                    ...
                    vutils.save_image(denorm(fakeGif.data), '%s/fakeGif_%03d_%d.png' % (self.outf, epoch, step), nrow=self.video_length)
