...
class Trainer(object):
    def __init__(self, config, image_loader, video_loader):
        ...
        self.enc_size = int(config.enc_size)
        ...
        self.L1_lambda = int(config.L1_lambda)
    ...
    def build_model(self):
        self.generator = mocogan.VideoGenerator(self.n_channels, self.dim_z_content, self.dim_z_category, self.dim_z_motion, self.video_length, self.enc_size)
    ...
    def train(self):
        ...
        for epoch in range(self.train_batches):
            ...
            for step in range(len(self.video_loader)):
                ...
                loss_G += self.L1_lambda * torch.mean(torch.abs(fakeGif[:, :, 0, :, :] - realIm))
